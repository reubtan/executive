...
def update_plot(selected_project, selected_floor_area):
    ...
    # Convert 'sold_at' to datetime and create a Year-Month column
    filtered_df['sold_at'] = pd.to_datetime(filtered_df['sold_at'])
    filtered_df['Year-Month Sold'] = filtered_df['sold_at'].dt.to_period('M').astype(str)  # Format as YYYY-MM
    ...
    # Create scatter plot with Year-Month Sold as x-axis
    fig = px.scatter(
        filtered_df,
    ...
    # Return figure, entry count, floor area options, and formatted summary statistics
    return fig, f"Number of resales: {num_entries}", floor_area_options, summary_stats
